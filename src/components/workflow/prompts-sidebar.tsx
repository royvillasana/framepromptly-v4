...
export function PromptsSidebar({ collapsed }: PromptsSidebarProps) {
  ...
  if (!hasPromptNodes && prompts.length === 0) {
    return (
      <div className={`border-r border-border bg-card transition-all duration-300 ${collapsed ? "w-14" : "w-80"}`}>
    ...
    <div className={`border-r border-border bg-card transition-all duration-300 ${collapsed ? "w-14" : "w-80"}`}>
      ...
      <div className="h-[calc(100vh-120px)]">
        {collapsed ? (
          ...
          <div className="p-2 space-y-2">
            {prompts.slice(0, 8).map((prompt) => (
              <Button
                ...
                variant="ghost"
                size="sm"
                className={`w-full h-8 p-0 ${currentPrompt?.id === prompt.id ? 'bg-primary/10' : ''}`}
          ...
          // Expanded view - show full prompt cards
