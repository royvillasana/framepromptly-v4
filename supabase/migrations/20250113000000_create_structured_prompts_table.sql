-- Migration: Create structured_prompts table for card-based prompt editing
-- Description: New table to store prompts as structured sections (role, context, task, etc.)
--              This enables card-based editing in the prompt library UI
...
CREATE INDEX idx_structured_prompts_user_id ON public.structured_prompts(user_id);
CREATE INDEX idx_structured_prompts_project_id ON public.structured_prompts(project_id);
CREATE INDEX idx_structured_prompts_library ON public.structured_prompts(is_library_prompt) WHERE is_library_prompt = true;
CREATE INDEX idx_structured_prompts_template ON public.structured_prompts(is_template) WHERE is_template = true;
CREATE INDEX idx_structured_prompts_tool ON public.structured_prompts(tool_name);
CREATE INDEX idx_structured_prompts_created_at ON public.structured_prompts(created_at DESC);
...
GRANT USAGE ON SEQUENCE public.structured_prompts_id_seq TO authenticated;
